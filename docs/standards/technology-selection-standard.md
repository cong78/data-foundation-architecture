...
Technology is selected against an architecture capability and evidence-based use case. Vendor features do not redefine Data Product Creation Contracts, authoritative metadata, policy intent, semantic context, telemetry, or service boundaries.
...
| Product Creation | Developer workspace, orchestration, transformation, quality, semantic tooling, CI/CD. | Product, contract and workload artifacts; exportable lineage; environment and release APIs. |
...
| Data Observability | OpenTelemetry collector and backend, data quality, lineage, incident and product-health tooling. | OTLP, OpenTelemetry conventions, portable lineage export, stable product and decision identifiers. |
...
| Interoperate | Use an independent client or receiver for the selected open interface and export portable source artifacts. |
