...
<div class="standards-map reference-map" role="img" aria-label="Three data contracts from source system through product creation to consumption">
  ...
  <section class="standards-map-lane lane-govern">
    ...
    <div class="standards-map-cell standards-map-focus"><strong>Source System Ingestion Contract</strong><p>Delivery, receipt, raw, validation, replay, and source-change controls.</p></div>
    <span class="standards-map-arrow" aria-hidden="true"></span>
  ...
  <section class="standards-map-lane lane-build">
    ...
    <span class="standards-map-arrow" aria-hidden="true"></span>
    <div class="standards-map-cell standards-map-focus"><strong>Data Product Creation Contract</strong><p>Transformation, meaning, quality, SLO, policy, output ports, and change.</p></div>
    <span class="standards-map-arrow" aria-hidden="true"></span>
...
| --- | --- | --- |
| Source system to ingestion | **Source System Ingestion Contract** | Delivery, source schema and meaning, keys, cadence, change notice, identity, reconciliation, replay, quarantine, raw retention, and validated source-aligned output. |
| Product inputs to product output | **Data Product Creation Contract** | Accepted input versions, transformation, output grain and semantics, quality, SLOs, classification, policy, lineage, ports, compatibility, go-live, rollback, and support. |
...
| Canonical repository and registry | Own immutable versions, state, owners, bindings, conditions, subscriptions, and history. |
| Contract compiler | Generate validation, compatibility, policy, interface, and telemetry configuration from the canonical artifact. |
...
4. Every governed consumer purpose has one Data Product Consumption Contract referencing an exact live product and contract version.
5. Sharing and AI use are consumption profiles, not contract types.
6. Product go-live and data release are impossible without matching runtime bindings, passing critical tests, approvals, and observable evidence.
7. Contract changes are assessed across connected contracts of all three types before release.
8. The same canonical artifacts drive documentation, tests, policy inputs, interface definitions, and telemetry correlation.
