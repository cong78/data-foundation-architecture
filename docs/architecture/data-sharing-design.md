...
Share only from a live, governed data product. Create a recipient-specific sharing contract that references the product contract, reduces the product to the minimum approved scope, and declares delivery, identity, freshness, quality, change, expiry, retention, and revocation behavior.
...
Treat a Unity Catalog **share** as the technical package and a **recipient** as the technical delivery identity. Neither is the business agreement, legal basis, data contract, or system of record for approvals.

## Solution at a Glance

```mermaid
flowchart TB
    PRODUCT["1 · Live data product<br/>owner · product contract · quality · health · stable port"]
    REQUEST["2 · Sharing request<br/>recipient · purpose · use case · legal basis · duration"]
    CONTRACT["3 · Sharing contract and agreement<br/>scope · identity · SLO · change · retention · revocation"]
...
| --- | --- | --- | --- |
| Product contract | Defines the live product schema, semantics, quality, freshness, classification, compatibility, and logical ports. | Product owner and contract registry. | Schema, comments, tags, views, quality evidence, and linked Unity Catalog product objects. |
| Sharing contract | Defines the recipient-facing package, minimized scope, delivery protocol, service levels, permitted use, change behavior, retention, and revocation. | Sharing owner and contract registry. | Share contents, exposed names, recipient properties, package view or table, and monitoring configuration. |
| Sharing agreement | Binds a recipient identity, purpose, legal basis, contract version, duration, entitlement, and obligations. | Data Service Portal and agreement registry. | Recipient object, share grant, authentication profile, activation status, and expiry automation. |
| Technical binding | Connects the approved agreement to exact provider objects and recipient configuration. | Data Sharing Service. | Unity Catalog share id, recipient id, shared objects, aliases, partitions or views, and client profile. |

The sharing contract narrows the product contract; it cannot silently weaken product quality, classification, semantic, or change controls. A sharing agreement references one published sharing-contract version and one recipient. Reusing one recipient object across agreements must not make scope or evidence ambiguous.
...
flowchart TB
    ...
    APPROVE["2 · Approve<br/>review and publish<br/><br/>classification · minimization · legal basis<br/>versioned contract · signed agreement"]
...
| Approval to binding | Data Sharing Service owner | Approved contract and agreement | Package and technical plan exactly match the approved versions. |
...
| --- | --- | --- |
| Draft | Product and recipient identified; purpose and requested scope recorded. | Required contract and agreement fields complete. |
...
## Sharing Contract Fields

The sharing contract is a portable artifact in the contract registry. It should use the enterprise [Data Contract Standard](../standards/data-contract-standard.md) and its Open Data Contract Standard extension mechanism rather than creating an unrelated schema.

| Field group | Minimum content |
| --- | --- |
| Identity | Contract id and version, product and product-contract references, sharing owner, recipient id, recipient trust domain, named-user or workload profile. |
...
kind: SharingContract
metadata:
  id: supplier-forecast-share
  version: 1.2.0
spec:
  productRef: supply-forecast@3.1.0
  productContractRef: supply-forecast-contract@3.1.0
  recipient:
    id: supplier-042
...
| Entitlement | Grant of a share to a recipient. | Exists only while the agreement is active and must be reconciled with expiry and revocation state. |
...
1. Resolve the approved product, product contract, sharing contract, agreement, recipient, and package version.
...
8. Mark the agreement active only after recipient acceptance and evidence capture.
...
| --- | --- |
| Additive field in provider product | Do not expose automatically when package fields are allowlisted. Review classification and issue a compatible sharing-contract version if required. |
...
| Tighten classification or prohibited use | Suspend affected access, reassess the agreement, and reactivate only after obligations are enforceable. |
| Reduce freshness, quality, availability, or history | Contract breach or breaking change according to the agreement; notify recipient and record acceptance or remediation. |
| Change recipient identity, trust domain, purpose, or legal entity | New or reapproved sharing agreement; do not silently reuse the old entitlement. |
...
- Product id and version, product contract id and version, sharing contract id and version.
- Agreement id, recipient id, recipient profile, share id, package version, and exposed object alias.
...
Revocation blocks future access; it cannot erase data that a recipient already exported. The sharing agreement must therefore combine technical revocation with contractual retention, deletion, backup, derived-data, and evidence obligations.

| Trigger | Required action |
| --- | --- |
| Agreement expiry | Revoke entitlement automatically, disable or remove recipient access as scoped, notify owners, and record evidence. |
...
| Purpose or ownership change | Suspend until the agreement and identity are reapproved. |
...
A sharing agreement may activate only when:
...
- Sharing contract and agreement reference exact product and contract versions.
...
- Keep product, sharing contract, agreement, recipient, and technical binding artifacts portable and version controlled.
...
### Increment 1: Establish Contract and Recipient Controls

- Define sharing contract, agreement, recipient, package, binding, identity, expiry, revocation, and evidence schemas.
...
| Contract projection | Define how sharing-contract fields generate package objects, aliases, recipient properties, entitlements, tests, and evidence. |
...
- Every share starts from a live product and references exact product and sharing-contract versions.
- Every active entitlement has an approved recipient, purpose, agreement, duration, and revocation target.
...
- Provider and recipient activity is auditable and correlated to product, contract, agreement, share, recipient, and outcome.
