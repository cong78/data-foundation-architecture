...
| Architecture policy | A versioned, machine-readable rule with a stable id, decision input, outcome, enforcement point, evidence requirement, owner, and lifecycle. |
| Consumer-aligned data product | A federated data product or governed view optimized for a defined consumption pattern or outcome while retaining stable ownership, contract, ports, lifecycle, and reuse by approved consumers. |
...
| Data Product Consumption Contract | The contract from one exact live product version to one approved consumer or recipient purpose, defining identity, port, channel, scope, controls, SLO, expiry, and revocation. |
| Data Product Creation Contract | The contract from accepted input-product versions to an aggregate or consumer-aligned product, defining transformation, semantics, quality, ports, SLOs, change, support, and go-live evidence. |
...
| Data product go-live | The accountable decision that a specific product and contract version has passed mandatory ownership, purpose, quality, security, lineage, observability, documentation, portability, support, and rollback gates and may be consumed. |
...
| Data product | A trusted, reusable dataset or data interface with clear ownership, documentation, quality expectations, access rules, lifecycle state, and consumer value. |
...
| Reference solution | A technology-specific implementation profile that maps products and services to the technology-neutral architecture without redefining its contracts, boundaries, or mandatory outcomes. |
...
| Source-aligned data product | A centrally managed, validated product that preserves source meaning and provenance while adding a stable contract, product descriptor, quality evidence, lineage, support, and governed ports. |
| Source System Ingestion Contract | The contract from a source system to the Data Ingestion Service, defining delivery, schema, provenance, reconciliation, replay, quarantine, source obligations, and the published source-aligned output. |
